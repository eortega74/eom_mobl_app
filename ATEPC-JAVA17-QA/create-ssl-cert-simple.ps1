...
if (Test-Path $keystorePath) {
    ...
    Write-Host "Keystore anterior eliminado" -ForegroundColor Yellow
}
...
try {
    $process = Start-Process -FilePath "keytool" -ArgumentList @(
        ...
        "-validity", "365",
        "-dname", "CN=localhost, OU=EOM QA, O=EOM, L=City, ST=State, C=US",
        "-storepass", "changeit",
        "-keypass", "changeit"
    ) -Wait -PassThru
    ...
    if ($process.ExitCode -eq 0 -and (Test-Path $keystorePath)) {
        ...
        Write-Host "Valido por: 365 dias" -ForegroundColor Yellow
        
        Write-Host "" 
        Write-Host "Ahora puedes iniciar la aplicacion con HTTPS en: https://localhost:8080" -ForegroundColor Green
        Write-Host "El navegador mostrara advertencia de seguridad (es normal con certificados self-signed)" -ForegroundColor Yellow
    ...
    Write-Host "Asegurate de tener Java instalado y keytool en el PATH" -ForegroundColor Yellow
    exit 1
}

Write-Host ""
Write-Host "Siguientes pasos:" -ForegroundColor Magenta
Write-Host "1. Compilar la aplicacion: mvn package -Dmaven.test.skip=true" -ForegroundColor White
...
Write-Host ""
Write-Host "Certificado SSL listo para EOM QA Framework" -ForegroundColor Green