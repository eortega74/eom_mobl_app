...
function generate_property(){
    new-item $env:ATE_JAVA17_PROPERTY_FILE_NAME -Force
    $Contents = "version=" + $env:ATE_JAVA17_RELEASE_VERSION + "`n" +
    ...
    [System.IO.File]::WriteAllLines($env:ATE_JAVA17_PROPERTY_FILE_NAME, $Contents, $Utf8NoBomEncoding)  
    cat $env:ATE_JAVA17_PROPERTY_FILE_NAME
}
...
function publish_to_artifactory($src_file, $target_path){
    echo $env:ATE_JAVA17_RELEASE_VERSION
    jf rt u $src_file $target_path --build-name $env:ATE_JAVA17_RELEASE_VERSION --build-number $env:ATE_JAVA17_RELEASE_VERSION --recursive=false > output.json
