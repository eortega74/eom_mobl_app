...
generate_package $env:PATH_FOR_FILES_TO_ZIP "package.zip"
...
if ($env:BRANCH_NAME -eq "master")
  ...
  publish_to_artifactory "$env:PROPERTY_FILE_NAME" "$env:JF_BUILD_REPO/$env:RELEASE_VERSION/$env:WEBAPP_FOLDER_NAME/"
  publish_to_artifactory "package.zip" "$env:JF_BUILD_REPO/$env:RELEASE_VERSION/$env:WEBAPP_FOLDER_NAME/"
  }
else
  {
  publish_to_artifactory "$env:PROPERTY_FILE_NAME" "$env:JF_BUILD_REPO/$env:RELEASE_VERSION_WITH_BRANCH_PREFIX/"
  publish_to_artifactory "$env:PROPERTY_FILE_NAME" "$env:JF_BUILD_REPO/$env:RELEASE_VERSION_WITH_BRANCH_PREFIX/$env:WEBAPP_FOLDER_NAME/"
