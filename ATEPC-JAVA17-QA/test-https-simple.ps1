...
Write-Host ""
Write-Host "Preparando para pruebas Selenium QA..." -ForegroundColor Magenta
...
if (Test-Path $qasPath) {
    ...
    if ($seleniumFiles.Count -gt 0) {
        ...
        Write-Host "  No se encontraron archivos Selenium especificos" -ForegroundColor Yellow
    }
} else {
...
Write-Host "Aplicacion HTTPS corriendo en: https://localhost:8080" -ForegroundColor White
...
Write-Host ""
Write-Host "Proximos pasos para QA:" -ForegroundColor Magenta
...
Write-Host "3. Ejecutar bateria de pruebas QA automatizadas" -ForegroundColor White

Write-Host ""
Write-Host "EOM QA Framework con HTTPS listo para testing" -ForegroundColor Green